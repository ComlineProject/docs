...
| 4 | **`comline-typescript`** — the pilot target repo | code / infra | *Done.* `ComlineProject/comline-typescript` holds `codegen/` (`comline-codegen-typescript`, extracted from `generation`) and `runtime/` (`@comline/runtime`, MPL-2.0, Node): the framing-agnostic contract, `DatagramFraming` + `JsonRpcFraming`, an in-memory `Transport`, and a `Client` / `Server` — the `Handshake` and framing wire formats byte-compatible with `comline-runtime` (comline-typescript#6, #7). The CLI and the conformance corpus depend on the generator by git rev. Next: the generator emits `<Proto>Client` / dispatcher against the package, then `libgen` (`lib` mode) and `std-extra`. CI runs a cargo job and a Node job. |
